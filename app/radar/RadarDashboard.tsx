...
export default function RadarDashboard({ seedOpportunities, seedSellers: sellers, seedRules: rules }: RadarDashboardProps) {
  ...
  const [showImport,setShowImport] = useState(false); const [showNew,setShowNew] = useState(false); const [linkEditingId,setLinkEditingId] = useState<string|null>(null); const [evaluationId,setEvaluationId] = useState<string|null>(null); const [notice,setNotice] = useState("");
  ...
  function saveLink(e:FormEvent<HTMLFormElement>){ e.preventDefault(); if(!linkEditingId) return; const raw=String(new FormData(e.currentTarget).get("url")||"").trim(); let url:string|null=null; if(raw){ try { const parsed=new URL(raw); if(!["http:","https:"].includes(parsed.protocol)) throw new Error("protocol"); url=parsed.href; } catch { setNotice("Link inválido. Use um endereço começando com http:// ou https://."); return; } } const overrides=readLinkOverrides(); overrides[linkEditingId]=url; localStorage.setItem(linkStorageKey,JSON.stringify(overrides)); persist(items.map(x=>x.id===linkEditingId?{...x,url,updatedAt:new Date().toISOString()}:x)); setNotice(url?`Link de ${linkEditingId} salvo.`:`Link de ${linkEditingId} removido.`); setLinkEditingId(null); }
  ...
  function createItem(e:FormEvent<HTMLFormElement>){ e.preventDefault(); const fd=new FormData(e.currentTarget); const raw:Record<string,unknown>={}; for(const [k,v] of fd.entries()) raw[k]=["askingPrice","fees","shipping","purchaseFees","maintenanceReserve","partsReserve","safetyMargin","sellingCosts","maxPurchase","quickResale","likelyResale","liquidity","condition","originality","completeness","iao","iam","ice"].includes(k)?n(v):String(v).trim(); const item=normalize(raw as Partial<Opportunity>,`RA-${String(nextOpportunityNumber(items, repoSeedIds)).padStart(3,"0")}`); persist([item,...items]); setShowNew(false); setNotice(`${item.id} salvo.`); }
  function updateEvaluation(e:FormEvent<HTMLFormElement>){ e.preventDefault(); if(!evaluationId) return; const current=items.find(x=>x.id===evaluationId); if(!current) return; const fd=new FormData(e.currentTarget); const raw:Record<string,unknown>={...current}; for(const [k,v] of fd.entries()) if(k!=="images") raw[k]=["askingPrice","fees","shipping","purchaseFees","maintenanceReserve","partsReserve","safetyMargin","sellingCosts","maxPurchase","quickResale","likelyResale","liquidity","condition","originality","completeness","iao","iam","ice"].includes(k)?n(v):String(v).trim(); const updated=normalize({...raw,id:current.id,origin:current.origin,createdAt:current.createdAt,updatedAt:new Date().toISOString()},current.id); const overrides=readLinkOverrides(); overrides[current.id]=updated.url; localStorage.setItem(linkStorageKey,JSON.stringify(overrides)); persist(items.map(x=>x.id===current.id?updated:x)); setEvaluationId(null); setNotice(`${current.id} reavaliado e salvo.`); }
  ...
  return <main>
    ...
    {evaluationId && (()=>{ const current=items.find(x=>x.id===evaluationId); return current ? <Modal title={`Avaliar ${current.id}`} onClose={()=>setEvaluationId(null)} large><form ref={evaluationFormRef} onSubmit={updateEvaluation}><SmartImport formRef={evaluationFormRef} initialUrl={current.url}/><OpportunityFields item={current}/><ModalActions onClose={()=>setEvaluationId(null)} submit="Salvar reavaliação"/></form></Modal> : null; })()}
